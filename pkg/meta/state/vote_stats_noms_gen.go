...
func (x NodeRoundStats) MarshalNoms(vrw nt.ValueReadWriter) (nodeRoundStatsValue nt.Value, err error) {
	...
	return nodeRoundStatsStructTemplate.NewStruct([]nt.Value{
...
func (x RoundStats) MarshalNoms(vrw nt.ValueReadWriter) (roundStatsValue nt.Value, err error) {
	...
	return roundStatsStructTemplate.NewStruct([]nt.Value{
		// x.Height (uint64)
		...
		if err == nil {
			...
			case "Validators":
				...
				if validatorsMap, ok := value.(nt.Map); ok {
					x.Validators = make(map[string]NodeRoundStats)
					validatorsMap.Iter(func(validatorsKey, validatorsValue nt.Value) (stop bool) {
						...
						x.Validators[string(validatorsKeyString)] = validatorsValueInstance
						return false
					})
				} else {
					err = fmt.Errorf(
						"RoundStats.UnmarshalNoms expected x.Validators to be a nt.Map; found %s",
						reflect.TypeOf(value),
					)
				}
...
func (x VoteStats) MarshalNoms(vrw nt.ValueReadWriter) (voteStatsValue nt.Value, err error) {
	...
	return voteStatsStructTemplate.NewStruct([]nt.Value{
		...
		if err == nil {
			...
			case "History":
				...
				if historyList, ok := value.(nt.List); ok {
					x.History = make([]RoundStats, 0, historyList.Len())
					historyList.Iter(func(historyItem nt.Value, idx uint64) (stop bool) {
						...
						x.History = append(x.History, historyItemInstance)
						return false
					})
				} else {
					err = fmt.Errorf(
						"VoteStats.UnmarshalNoms expected x.History to be a nt.List; found %s",
						reflect.TypeOf(value),
					)
				}
